...
#   pragma warning restore 67
        ...
        private void HandleException(Exception ex, string errorPrefix = null)
        {
            ...
            if (IsUriEmptyError(ex, errorPrefix))
            {
                Program.Print("[Error] Input a minerAddress AND privateKey into the B0xToken.conf file in order to mine");
                return;
            }
        ...
        private bool IsUriEmptyError(Exception ex, string errorPrefix)
        {
            // Check if this matches the specific error pattern
            return  ex.Message.Contains("Invalid URI: The URI is empty");
        }
    }
}