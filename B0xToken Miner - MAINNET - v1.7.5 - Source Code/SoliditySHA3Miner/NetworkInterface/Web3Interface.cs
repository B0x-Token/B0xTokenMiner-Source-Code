            ...
            {  //this goes 1000+ lines
               ...
               //  OnStopSolvingCurrentChallenge(this);
                ...
                try
                {
                    ...
                    if (areEqual)
                    {
                        Program.Print("This is to check for close to end of mining period to submit MinSolvesperMint if nessessary");
                        Program.Print("This is to check for close to end of mining period to submit MinSolvesperMint if nessessary");
                        Program.Print("This is to check for close to end of mining period to submit MinSolvesperMint if nessessary");
                    ...
                    if ((timeBetweenAnswers > tststs || prevEpcohToreadjust < epochNumber5555) && epochNumber5555 != 2017)
                    {
                        ...
                        if (save != save2)
                        {
                            Program.Print(string.Format("[INFO] Submission cancelled, someone has solved this challenge. Try lowering MinSolvesperMint variable to submit before them."));
                            Task.Delay(300).Wait();
                        ...
                        try
                        {
                            ...
                            if (mintNFT == true)
                            {
                                ...
                                catch (Exception e)
                                {
                                    ...
                                    if (e.ToString().Contains("Must have valid solve in the answers"))
                                    {
                                        Program.Print("❌ ❌  Must have valid solve in the answers we will fix will restart normal mining in 3 seconds ❌ ");
                                        ...
                                        Program.Print($"WROTE {filePath2f} & {filePath3}&{filePath4} {currentCounter}");

                                        File.WriteAllText(filePath2f, currentCounter.ToString());
                                        File.WriteAllText(filePath3, currentCounter.ToString());
                                        File.WriteAllText(filePath4, currentCounter.ToString());
